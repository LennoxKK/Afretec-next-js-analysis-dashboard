...
import { FiSend, FiDownload, FiTrendingUp, FiMessageSquare } from 'react-icons/fi';
...
export const Chat: React.FC<ChatProps> = ({ messages, onSendMessage, isLoading }) => {
  ...
  const exampleQueries = [
    {
      title: "Malaria trends by demographic",
      query: "Show malaria cases by age group and gender as a stacked bar chart for the last 5 years",
      icon: FiMessageSquare,
      color: "red"
    },
    {
      title: "Disease comparison",
      query: "Compare dengue and chikungunya cases by month in Southeast Asia",
      icon: FiMessageSquare,
      color: "blue"
    },
    {
      title: "Vaccination rates",
      query: "Display COVID-19 vaccination rates by country on a world map",
      icon: FiMessageSquare,
      color: "green"
    }
  ];
        ...
        <div className="max-w-6xl mx-auto space-y-4">
          {uniqueMessages.length === 0 ? (
            ...
            >
              ...
              <p className="text-gray-700 max-w-lg leading-relaxed mb-6 text-base">
                Transform disease data into actionable insights. Request visualizations, analyze trends, and explore patterns through interactive charts.
              </p>
              ...
              <div className="w-full max-w-md space-y-3">
                {exampleQueries.map((example, index) => (
                  <motion.div
                    ...
                    className="p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 cursor-pointer transition-all hover:shadow-xs group"
                  >
                    <div className="flex items-center">
                      <div className={`w-6 h-6 rounded-md bg-${example.color}-100 flex items-center justify-center mr-3`}>
                        <example.icon className={`w-3 h-3 text-${example.color}-600`} />
                      </div>
      ...
      <div className="sticky bottom-0 bg-white border-t border-gray-200 px-4 py-3">
        ...
        >
          <form onSubmit={handleSend} className="relative">
            <div className="relative bg-white rounded-lg border border-gray-300 focus-within:border-blue-500 transition-colors">
              <textarea
                rows={1}
                value={input}
                ...
                className="w-full p-3 pr-12 bg-transparent border-0 focus:ring-0 resize-none text-sm text-gray-900 placeholder-gray-500 disabled:opacity-50 rounded-lg"
              ...
              <motion.button
                ...
                className={`absolute right-2 top-1/2 transform -translate-y-1/2 p-2 rounded-md transition-all ${
            ...
            <div className="flex items-center justify-between mt-2">
              <p className="text-xs text-gray-500">
                ...
                <button 
                  type="button"
                  onClick={() => handleExampleClick("Compare malaria vs dengue cases by region")}
                  className="ml-1 hover:text-blue-600 hover:underline"
                >
                  &quot;Compare malaria vs dengue cases by region&quot;
                </button>
              </p>
